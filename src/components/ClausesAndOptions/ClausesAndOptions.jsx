                ...
                <div className="template-options level-1">
                    ...
                    <div className="template-options level-2">
                        {options?.map(({ summary, id: subSectionId, groupClauses }, subSectionIndex) => {
                            return <>
                                <p key={`subsection-${subSectionIndex}`} onClick={() => optionSelectHandler("subSection", subSectionId, null, null)} className={`${optionSelected?.subSectionId === subSectionId ? "active" : ""}`}>{summary}</p>
